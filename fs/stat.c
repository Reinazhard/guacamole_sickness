...
SYSCALL_DEFINE2(newfstat, unsigned int, fd, struct stat __user *, statbuf)
{
	...
	/*
	 * Only meaningful when the syscall succeeded: on error cp_new_stat() never
	 * ran, so *statbuf is whatever the caller had there and must not be touched.
	 */
...
#ifdef CONFIG_KSU_SUSFS
	// 32-bit su: on arm64 this is where compat stat() lands (unistd32.h
	// wires __NR_fstatat64 here), so zygote32 needs it hooked too.
	if (likely(susfs_is_current_proc_no_su()))
		goto orig_flow_fstatat64;
