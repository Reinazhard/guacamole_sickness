...
void susfs_add_sus_path_loop(void __user **user_info)
{
	...
	SUSFS_LOGI(
		"target_pathname: '%s', is successfully added to LH_SUS_PATH_LOOP\n",
	...
	list_for_each_entry_rcu(cursor, &LH_SUS_PATH_LOOP, list) {
		if (!kern_path(cursor->target_pathname, 0, &path)) {
			...
			if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
				...
				SUSFS_LOGI(
					"re-flag AS_FLAGS_SUS_PATH on path '%s', fi->inode.i_ino: '%lu', fi->inode.i_mapping->flags: 0x%lx\n",
				...
				set_bit(AS_FLAGS_SUS_PATH,
					&inode->i_mapping->flags);
				SUSFS_LOGI(
					"re-flag AS_FLAGS_SUS_PATH on path '%s', inode->i_ino: '%lu', inode->i_mapping->flags: 0x%lx\n",
...
void susfs_set_hide_sus_mnts_for_non_su_procs(void __user **user_info)
{
	...
	if (copy_from_user(&info,
			   (struct st_susfs_hide_sus_mnts_for_non_su_procs __user
				    *)*user_info,
			   sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	...
	info.err = 0;
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_hide_sus_mnts_for_non_su_procs __user
				    *)*user_info)
				  ->err,
			 &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
...
static int
susfs_mark_inode_sus_kstat(char *target_pathname,
			   struct st_susfs_sus_kstat_hlist *new_entry)
{
	...
	if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
		...
		SUSFS_LOGI(
			"flagged AS_FLAGS_SUS_KSTAT on pathname: '%s', is_fuse: %d, fi->inode.i_sb->s_dev: %u, fi->nodeid: %llu, fi->inode.i_ino: %lu, fi->inode.i_mapping->flags: 0x%lx\n",
			target_pathname, new_entry->is_fuse,
			fi->inode.i_sb->s_dev, fi->nodeid, fi->inode.i_ino,
			fi->inode.i_mapping->flags);
		err = 0;
		goto out_path_put_path;
	}
	...
	SUSFS_LOGI(
		"flagged AS_FLAGS_SUS_KSTAT on pathname: '%s', is_fuse: %d, inode->i_sb->s_dev: %u,  inode->i_ino: %lu, inode->i_mapping->flags: 0x%lx\n",
		target_pathname, new_entry->is_fuse, inode->i_sb->s_dev,
		inode->i_ino, inode->i_mapping->flags);
...
void susfs_add_sus_kstat(void __user **user_info)
{
	...
	new_entry =
		kzalloc(sizeof(struct st_susfs_sus_kstat_hlist), GFP_KERNEL);
	if (!new_entry) {
		info.err = -ENOMEM;
		goto out_copy_to_user;
	}
	...
	SUSFS_LOGI(
		"is_fuse: %d, is_statically: '%d', target_ino: '%lu', target_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', spoofed_size: '%llu', spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%ld', spoofed_mtime_tv_nsec: '%ld', spoofed_ctime_tv_nsec: '%ld', spoofed_blksize: '%lu', spoofed_blocks: '%llu', is successfully added to SUS_KSTAT_HLIST\n",
		new_entry->is_fuse, new_entry->info.is_statically,
...
void susfs_update_sus_kstat(void __user **user_info)
{
	...
	hash_for_each_safe(SUS_KSTAT_HLIST, bkt, tmp_hlist_node, tmp_entry,
			   node) {
		if (!strcmp(tmp_entry->info.target_pathname,
			    info.target_pathname)) {
			...
			SUSFS_LOGI(
				...
				tmp_entry->target_ino, new_entry->target_ino,
				new_entry->info.target_pathname);
			hash_del_rcu(&tmp_entry->node);
			hash_add_rcu(SUS_KSTAT_HLIST, &new_entry->node,
				     info.target_ino);
					    ...
					    struct kstat *stat)
{
	...
	if (inode->i_sb->s_magic == FUSE_SUPER_MAGIC) {
		...
		if (!test_bit(AS_FLAGS_SUS_KSTAT,
			      &fi->inode.i_mapping->flags) ||
...
out_spoof_kstat:
	...
	hash_for_each_possible_rcu(SUS_KSTAT_HLIST, entry, node, target_ino) {
		if (entry->target_dev == target_dev &&
			...
			SUSFS_LOGI(
				"spoofing kstat for path: %s, target_ino: %lu, target_dev: %u\n",
				entry->info.target_pathname, target_ino,
				target_dev);
...
out_spoof_kstat:
	...
	hash_for_each_possible_rcu(SUS_KSTAT_HLIST, entry, node, target_ino) {
		if (entry->target_dev == target_dev &&
		    entry->is_fuse == is_fuse) {
			SUSFS_LOGI(
				"spoofing kstat for target_ino: %lu, target_dev: %u\n",
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info)
{
	...
	strscpy(fake_cmdline_or_bootconfig, info->fake_cmdline_or_bootconfig,
...
void susfs_add_open_redirect(void __user **user_info)
{
	...
	if (!redirected_inode || !redirected_inode->i_mapping) {
		SUSFS_LOGE(
			"redirected_inode || redirected_inode->i_mapping is NULL\n");
		info.err = -ENOENT;
	...
	if (redirected_inode->i_sb->s_magic == FUSE_SUPER_MAGIC ||
		...
		SUSFS_LOGE(
			"FUSE fs is not supported for open_redirect feature\n");
		info.err = -EINVAL;
		goto out_path_put_target_path;
	}

	new_entry_target = kzalloc(sizeof(struct st_susfs_open_redirect_hlist),
				   GFP_KERNEL);
	if (!new_entry_target) {
		info.err = -ENOMEM;
		goto out_path_put_target_path;
	...
	new_entry_redirected = kzalloc(
		sizeof(struct st_susfs_open_redirect_hlist), GFP_KERNEL);
	...
	new_entry_redirected->spoofed_mnt_id =
		real_mount(target_path.mnt)->mnt_id;
...
struct filename *susfs_open_redirect_spoof_do_sys_openat(struct inode *inode)
{
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node,
		...
		if (!entry->reversed_lookup_only &&
			...
			SUSFS_LOGI(
				"redirect path '%s' to '%s', uid_scheme: %d\n",
				entry->info.target_pathname,
				entry->info.redirected_pathname,
				entry->info.uid_scheme);
			new_filename =
				getname_kernel(entry->info.redirected_pathname);
					   ...
					   char __user *buffer, int buflen)
{
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node,
		...
		if (entry->reversed_lookup_only &&
			...
			if (copy_to_user(
				    buffer, entry->info.redirected_pathname,
						...
						char **out_spoofed_name)
{
	...
	if (!out_spoofed_name || *out_spoofed_name != NULL) {
		SUSFS_LOGE(
			"out_spoofed_name cannot be NULL and *out_spoofed_name has to be NULL\n");
		return -EINVAL;
	}
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node,
		...
		if (entry->reversed_lookup_only &&
		    entry->target_dev == inode->i_sb->s_dev) {
			SUSFS_LOGI(
				"spoof maps ino/dev/name for redirected path: '%s'\n",
				entry->info.target_pathname);
			*out_ino = entry->redirected_ino;
...
void susfs_get_enabled_features(void __user **user_info)
{
	...
	if (!info) {
		...
		/*
		 * info is NULL, so the error has to be reported through the
		 * caller's structure directly.  Falling through to
		 * out_copy_to_user would dereference the NULL info first.
		 */
		(void)copy_to_user(
