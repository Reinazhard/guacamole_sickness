...
#define VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT \
	0x80000000 /* used for mounts that are unshared by ksu process */
#define DEFAULT_KSU_MNT_ID \
	2000000000 /* used for mounts created or single cloned by ksu process */
...
#define SUSFS_IS_INODE_SUS_MAP(inode)                                   \
	inode &&                                                        \
		inode->i_mapping &&unlikely(test_bit(                   \
			AS_FLAGS_SUS_MAP, &inode->i_mapping->flags)) && \
		susfs_is_current_proc_umounted_app()

#define SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(inode) \
	inode && inode->i_mapping &&unlikely(test_bit(        \
			 AS_FLAGS_OPEN_REDIRECT, &inode->i_mapping->flags))
