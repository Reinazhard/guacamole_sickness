		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	for (; nr_pages != 0; index++, nr_pages--) {
		...
		zram_set_flag(zram, index, ZRAM_IDLE);
		zram_slot_unlock(zram, index);
		if (zram_read_page(zram, page, index, NULL, false, false)) {
			zram_slot_lock(zram, index);
				  ...
				  u32 index, int offset, struct bio *bio, bool access)
{
	...
	if (!page)
		return -ENOMEM;
	ret = zram_read_page(zram, page, index, bio, true, access);
	if (likely(!ret))
			...
			u32 index, int offset, struct bio *bio, bool access)
{
	if (is_partial_io(bvec))
		return zram_bvec_read_partial(zram, bvec, index, offset,
						bio, access);
	return zram_read_page(zram, bvec->bv_page, index, bio, false, access);
}
