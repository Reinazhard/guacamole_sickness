...
restart:
	for (i = pte_index(start), addr = start; addr != end; i++, addr += PAGE_SIZE) {
		...
		pfn = get_pte_pfn(pte[i], args->vma, addr, pgdat);
		if (pfn == -1)
			continue;
...
restart:
	...
	for (i = pmd_index(start), addr = start; addr != end; i++, addr = next) {
		...
		if (!walk->force_scan && !test_bloom_filter(walk->lruvec, walk->seq, pmd + i))
			continue;
		...
		update_bloom_filter(walk->lruvec, walk->seq + 1, pmd + i);
	}
...
bool lru_gen_look_around(struct page_vma_mapped_walk *pvmw)
{
	...
	for (i = 0, addr = start; addr != end; i++, addr += PAGE_SIZE) {
		...
		pfn = get_pte_pfn(pte[i], vma, addr, pgdat);
		if (pfn == -1)
			continue;
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	...
	if ((lru_gen_enabled() || lru_gen_switching()) && !global_reclaim(sc)) {
	...
	if (!pgdat->kswapd) {
		...
		if (IS_ERR(pgdat->kswapd)) {
			...
			ret = kfifo_alloc(&pgdat->kcompress_fifo,
					...
					GFP_KERNEL);
			if (ret) {
				pr_err("Failed to allocate kcompress_fifo on node %d\n", nid);
			} else {
