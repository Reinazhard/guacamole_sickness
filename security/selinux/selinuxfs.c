...
static ssize_t my_write_context(struct file *file, char *buf, size_t size)
{
	...
	length = security_context_to_sid(&fake_state, buf, size, &sid,
					 GFP_KERNEL);
	if (length)
		goto out;
...
#endif // #ifdef CONFIG_KSU_SUSFS
	[SEL_CREATE] = sel_write_create,  [SEL_RELABEL] = sel_write_relabel,
	[SEL_USER] = sel_write_user,	  [SEL_MEMBER] = sel_write_member,
#ifdef CONFIG_KSU_SUSFS
...
static ssize_t my_write_access(struct file *file, char *buf, size_t size)
{
	...
	length = security_context_str_to_sid(&fake_state, scon, &ssid,
					     GFP_KERNEL);
	if (length)
		goto out;

	length = security_context_str_to_sid(&fake_state, tcon, &tsid,
					     GFP_KERNEL);
	if (length)
		goto out;
